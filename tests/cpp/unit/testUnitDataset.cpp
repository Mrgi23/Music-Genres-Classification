...
class MockPreprocessor : public Preprocessor {
    public:
        ...
        MOCK_METHOD(void, run, (const fs::path& filePath, vector<vector<float>>& spectrogram, vector<vector<float>>& mfcc), (override));
};
...
TEST_F(TestAudioDataset, getValidOutput) {
    ...
    size_t specSize = 1025;
    size_t mfccSize = 13;
    vector<vector<float>> mockSpectrogram = randomMatrix(numFrames, specSize);
    vector<vector<float>> mockMfcc = randomMatrix(numFrames, mfccSize);
    EXPECT_CALL(*mockPreprocessor, run(_, _, _)).WillOnce([&](
        const string&,
    ...
    // Test the result.
    ASSERT_EQ(sample.data.numel(), numFrames * (specSize + mfccSize)) << "Invalid size of the samle data.";
