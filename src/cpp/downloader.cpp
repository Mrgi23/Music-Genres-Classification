...
Downloader::Downloader(const fs::path& rootPath, const std::string& url)
...
void Downloader::downloadAndExtract()
{
  if (!isExisting())
  {
    if (downloadFromUrl(m_archiveFile, m_url))
      ...
      throw std::invalid_argument("Downloader::downloadAndExtract: URL: " + m_url + " is invalid.");
  }
}

fs::path Downloader::rootPath() const
{
  return m_rootPath;
}

bool Downloader::downloadFromUrl(const fs::path& filePath, const std::string& url)
{
