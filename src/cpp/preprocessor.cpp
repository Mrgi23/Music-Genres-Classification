...
void Preprocessor::run(
    ...
    for (uint i = 0; i < numFrames; ++i) {
        ...
        // Compute frame logaritmic spectrogram.
        vector<float> specFrame;
        for (uint j = 0; j < stftVector->length; j++) { specFrame.push_back(10.0f * log10f(stftVector->norm[j])); }
        spectrogram.push_back(specFrame);

        // Compute frame MFCC.
        fvec_t * mfccVector = new_fvec(nmfcc);
        aubio_mfcc_do(mfccObject, stftVector, mfccVector);
        vector<float> mfccFrame;
        for (uint j = 0; j < nmfcc; j++) { mfccFrame.push_back(mfccVector->data[j]); }
        mfcc.push_back(mfccFrame);
