...
AudioDataset::AudioDataset(const fs::path& rootPath, Preprocessor* preprocessor)
  : m_preprocessor(preprocessor)
{
  ...
  {
    std::vector<std::pair<fs::path, torch::Tensor>> tasks;
    ...
    {
      std::string genre = genreEntry.filename().string();
...
c10::Dict<int64_t, std::string> AudioDataset::classes() const
{
  return m_classes;
}
...
AudioSubset::AudioSubset(AudioDataset* dataset, std::vector<size_t> indices)
  : m_dataset(dataset), m_indices(indices), m_stackedData(torch::Tensor())
{
}

AudioSubset::~AudioSubset() = default;

AudioDataset* AudioSubset::dataset() const
{
  return m_dataset;
}

torch::Tensor AudioSubset::stackedData() const
{
  if (m_stackedData.defined())
    return m_stackedData;

  std::vector<torch::Tensor> allData;
