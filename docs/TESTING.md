...
#### 1. Linux/WSL
```sh
sudo apt update && sudo apt install -y git cmake clang make libaubio-dev libcurl4-openssl-dev libzip-dev lcov llvm python3.10 python3.10-dev python3.10-venv
...
#### 2. macOS
```sh
brew update && brew install aubio git cmake clang curl make lcov libzip llvm python@3.10
...
- Test **Preprocessor** for correct feature extraction, including **spectrogram** and **MFCCs** computations.
